...
describe('PlotxToken Test Cases', function() {
  ...
  it("Should not allow unauthorized address to call functions with onlyOperator", async function () {
    ...
    it("Should not update operator if address provided is 0x000...000", async function () {

    let newPlotTok = await PlotusToken.new("PLOT1", "PLOT1", 18, user1, user1);
    //changeOperator

		let nullAddress = "0x0000000000000000000000000000000000000000";
    functionSignature = encode("changeOperator(address)", nullAddress);
  ...
  it("Should be able to call functions with onlyOperator via meta transaction", async function () {
    ...
    let newPlotTok = await PlotusToken.new("PLOT1", "PLOT1", 18, user1, user1);
	  await newPlotTok.mint(user3,toWei(100));
    ...
    await assertRevert(newPlotTok.transfer(user1,toWei(3000), {from:user3}));
    await newPlotTok.approve(user1,toWei(3000), {from:user3})
    await assertRevert(newPlotTok.transferFrom(user3, user1,toWei(3000)));
